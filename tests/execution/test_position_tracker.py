...
def buy_plan() -> TradePlan:
    return TradePlan(approved=True, reason='test_buy', symbol='BTC', side='BUY', amount=10.0, stop_loss=95.0, take_profit=110.0)
...
def snapshot(symbol: str = 'BTC', last: float = 100.0, timestamp: datetime | None = None) -> MarketSnapshot:
    return MarketSnapshot(symbol=symbol, bid=last - 0.5, ask=last + 0.5, last=last, timestamp=timestamp or datetime(2026, 6, 25, 12, 0, tzinfo=timezone.utc))
...
def test_tracked_position_defaults_do_not_enable_managed_stops():
    ...
    assert position.trailing_stop_net_buffer_percent == 0.0
    assert position.managed_stop_protection_type is None
    assert position.last_stop_update_metadata is None
...
def test_position_tracker_uses_trade_plan_managed_stop_settings_for_new_position():
    ...
    assert tracked_position.trailing_stop_net_buffer_percent == 0.1
...
def test_position_tracker_keeps_persisted_managed_stop_settings_on_restore():
    ...
    assert restored_position.trailing_stop_net_buffer_percent == 0.2
    assert restored_position.managed_stop_protection_type == 'trailing'
...
def test_position_tracker_moves_buy_stop_to_break_even():
    ...
    assert close_signals[0].metadata['managed_stop_protection_type'] == 'break_even'
