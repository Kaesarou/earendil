...
from app.brokers.base import ClosePositionSubmission, ClosePositionSubmissionUnknownError
...
class FakeRunner:
    ...
    def has_pending_kind(self, kind): return any(item['kind'] == kind for item in self.tasks)
...
class FakeRiskManager:
    def __init__(self): self.closed_symbols = []
...
class FakeMarketDataCoordinator:
    def mark_fallback_failed(self, symbols): return None
    def mark_fallback_succeeded(self, symbols): return None
...
def position(position_id, symbol):
    return TrackedPosition(
        position_id=position_id, symbol=symbol, side='BUY', amount=500.0,
        entry_price=100.0, stop_loss=99.0, take_profit=102.0, opened_at=NOW,
    )

def signal(position_id, symbol):
    return PositionCloseSignal(
        position_id=position_id, symbol=symbol, side='BUY', exit_price=98.9,
...
def build_coordinator(tmp_path):
    ...
    coordinator = AsyncBrokerOperationsCoordinator(
        runner=runner, execution_broker=broker, rest_market_data=SimpleNamespace(),
        executor=FakeExecutor(), position_tracker=tracker, risk_manager=risk,
        position_store=position_store, pending_close_store=pending_store,
        cooldown_store=SimpleNamespace(), trade_journal=journal,
...
def completion_for(task, *, value=None, error=None):
    return BrokerTaskCompletion(
        task_id=task.get('task_id') or task['kind'], kind=task['kind'],
        lane=BrokerTaskLane.CLOSE, context=task['context'], value=value, error=error,
    )
...
def test_repeated_signals_create_one_request_and_one_post(tmp_path):
    ...
    assert coordinator.submit_close(signal=signal('position-a', 'BTC'), source='websocket_position_guard')
    assert not coordinator.submit_close(signal=signal('position-a', 'BTC'), source='websocket_position_guard')
    assert len(runner.tasks) == 1
    assert [event for event, _ in journal.events].count('position_close_requested') == 1
...
def test_nearby_closes_are_both_queued_without_waiting_for_confirmation(tmp_path):
    ...
    assert coordinator.submit_close(signal=signal('position-a', 'BTC'), source='websocket_position_guard')
    assert coordinator.submit_close(signal=signal('position-b', 'ETH'), source='websocket_position_guard')
    assert [task['task_id'] for task in runner.tasks] == [
        'close_position:position-a', 'close_position:position-b'
    ]

def test_accepted_submission_keeps_position_and_risk_until_portfolio_absence(tmp_path):
    coordinator, runner, journal, tracker, risk, pending_store, position_store = build_coordinator(tmp_path)
    coordinator.submit_close(signal=signal('position-a', 'BTC'), source='websocket_position_guard')
    task = runner.tasks[0]
    coordinator.handle_completion(completion_for(task, value=task['operation']()), now=NOW, latest_snapshots={})
...
def test_ambiguous_submission_never_retries_and_confirms_from_one_absence(tmp_path):
    ...
    error = ClosePositionSubmissionUnknownError(
        position_id='position-a', submitted_at=NOW, cause=TimeoutError('network timeout')
    )
    coordinator.handle_completion(completion_for(task, error=error), now=NOW, latest_snapshots={})
    assert len(runner.tasks) == 1
    ...
    coordinator.handle_completion(
        BrokerTaskCompletion(
            task_id='reconcile-1', kind='position_reconciliation',
            lane=BrokerTaskLane.STANDARD, context=scheduled['context'],
    ...
    assert [item.position_id for item in position_store.load_open_positions()] == ['position-b']
