...
def make_candidate(symbol: str) -> TradeCandidate:
    ...
    snapshot = MarketSnapshot(symbol=symbol, bid=99.95, ask=100.05, last=100.0, timestamp=now)
    candle = Candle(
        symbol=symbol,
...
def test_candidate_economics_estimator_estimates_equity_candidate_costs():
