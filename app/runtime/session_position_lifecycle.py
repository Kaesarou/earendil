...
from app.market.session_rules import FORCE_CLOSE_BEFORE_SESSION_END, TradingSessionDecision
from app.persistence.position_store import PositionStore
from app.persistence.trade_cooldown_store import TradeCooldownStore
from app.risk.risk_manager import RiskManager
from app.runtime.position_lifecycle import register_trade_cooldown_for_closed_position
    ...
    *,
    ...
    for position in position_tracker.open_positions_snapshot():
        ...
        close_signal = PositionCloseSignal(
            ...
            metadata={
                'session_decision_reason': session_decision.reason,
                'time_until_session_end_minutes': (
                    session_decision.time_until_session_end_minutes
                    if session_decision.time_until_session_end_minutes is not None
                    else -1.0
                ),
            },
        )
        try:
            ...
            if is_broker_authorization_error(exc):
                raise
            logger.exception(
                'Session position close error | symbol=%s | position_id=%s | error=%s',
                symbol,
