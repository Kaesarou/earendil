...
def _cooldown_payload(*, entry, cooldown_config) -> dict:
    ...
    if entry.close_reason == CloseReason.STOP_LOSS:
        symbol_lock_expires_at = entry.symbol_lock_expires_at(
            cooldown_config
        )
    return {
        'entry': entry,
...
def register_trade_cooldown_for_closed_position(
    *,
    ...
    if closed_position is None:
        return
    cooldown_config = risk_manager.risk_profile_for(
    ...
    logger.info(
        'Trade cooldown registered | source=bot_close | symbol=%s | '
...
def register_trade_cooldown_for_missing_position(
    *,
    ...
    logger.info(
        'Trade cooldown registered | source=broker_reconciliation | '
        'symbol=%s | side=%s | reason=%s | lock_scope=%s | '
        'expires_at=%s',
        saved_entry.symbol,
        saved_entry.side,
        saved_entry.close_reason.value,
    ...
    *,
    ...
    for close_signal in close_signals:
        try:
            ...
            closed_session_key = risk_manager.record_close_position(
                close_signal.symbol
            )
            if position_store is not None:
                try:
                    position_store.delete_open_position(
                        close_signal.position_id
                    )
                except Exception as exc:
                    logger.exception(
                        'Position persistence delete error | '
            ...
            if is_broker_authorization_error(exc):
                raise
            logger.exception(
                'Position close error | symbol=%s | position_id=%s | '
                'reason=%s | error=%s',
                symbol,
                close_signal.position_id,
                close_signal.reason,
...
def _record_managed_stop_updates(
    *,
    snapshot: MarketSnapshot,
    position_tracker: PositionTracker,
    ...
    for update in position_tracker.consume_managed_stop_updates():
        ...
        try:
            ...
            logger.exception(
                'Position persistence managed-stop update error | '
                'position_id=%s | error=%s',
    ...
    *,
    ...
    for position in position_tracker.open_positions_snapshot():
        ...
        try:
            ...
            logger.exception(
                'Position persistence delete error | position_id=%s | '
                'error=%s',
                removed_position.position_id,
                exc,
            )
            ...
            try:
                ...
                if is_broker_authorization_error(exc):
                    logger.critical(
                        ...
                        'Stopping before continuing | position_id=%s | '
                        'symbol=%s | error=%s',
                        position.position_id,
                        position.symbol,
                        exc,
                    ...
                    raise
                logger.exception(
                    'Failed to restore broker instrument mapping | '
