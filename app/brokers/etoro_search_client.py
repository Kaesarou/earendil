...
class EtoroSearchClient(EtoroClient):
    def get_market_snapshot(self, symbol: str) -> MarketSnapshot:
        ...
        instrument = self._find_exact_market_data_item(symbol, self._extract_items(payload))
        return self._to_market_snapshot_from_search_item(symbol, instrument)

    def _find_exact_market_data_item(self, symbol: str, items: list[dict]) -> dict:
        normalized_symbol = symbol.strip().upper()
        for item in items:
            item_symbol = item.get('internalSymbolFull')
            if item_symbol is not None and str(item_symbol).strip().upper() == normalized_symbol:
                return item
