...
TP_PROBABILITY_MODEL_VERSION = 'heuristic_v1'
...
class TpBeforeSlProbabilityEstimator:
    ...
    def estimate(
        ...
        *,
        ...
        feasibility_score = _bounded(tp_feasibility.runway_score, 0.0, 100.0)
        ...
        trend_score = self._score_low_value_is_bad(
            ...
            value=abs(_optional_float(metadata.get('trend_strength_percent')) or 0.0),
        ...
        component_scores = {
            ...
            'movement_freshness_score': round(movement_freshness_score, 4),
        ...
        probability = self.config.base_probability + self.config.probability_range * (raw_score / 100)
        probability = _bounded(probability, self.config.min_probability, self.config.max_probability)

        reason_components.extend(_probability_reason_components(component_scores))
        if tp_feasibility.reason_components:
            reason_components.extend(f'tp_feasibility:{component}' for component in tp_feasibility.reason_components)
...
class CandidateTpProbabilityEvaluator:
    ...
    def evaluate(self, evaluated_candidate: EvaluatedTradeCandidate) -> EvaluatedTradeCandidate:
...
def _append_rank_reason(rank_reason: str, estimate: TpBeforeSlProbabilityEstimate) -> str:
    suffix = (
        f'tp_before_sl_probability={estimate.tp_before_sl_probability:.2f},'
...
def _close_quality(candidate: TradeCandidate) -> float | None:
    metadata = candidate.signal.metadata or {}
    close_position_percent = _optional_float(metadata.get('close_position_percent'))
...
def _probability_reason_components(component_scores: dict[str, float | None]) -> list[str]:
    components: list[str] = []
