...
def main() -> None:
    ...
    logger.info(
        'Starting Eärendil | broker=%s | strategy_profile=%s | watchlist=%s | journal_detail=%s',
        settings.broker,
    ...
    trade_journal = build_analysis_journal(settings)
    ...
    try:
        ...
        while True:
            try:
                ...
                symbols_to_fetch, session_decisions, started_symbols, completed_session_keys = (
                ...
                for symbol in started_symbols:
                    ...
                    trade_journal.write(
                        'session_started',
                        {'symbol': symbol, 'session_decision': session_decisions[symbol]},
                    )
                for symbol, session_decision in session_decisions.items():
                    if not session_decision.collect_snapshots:
                        trade_journal.write(
                            'session_state',
                            {'symbol': symbol, 'session_decision': session_decision},
                        )
